...
export const saveState = async (id) => {
  ...
  await Promise.all(writePromise).then(console.log('Packets Written ' + message));
 // await wait(1000);
  const response = await BluetoothSerial.readFromDevice(id);
  await Promise.all(response).then(console.log('Response Recieved ' + response));
...
export const getConfiguration = async (id) => {
  ...
  try {
    ...
    await Promise.all(writePromise).then(console.log('Packets Written ' + newBuffer));
    await wait(1000);
    const response = await BluetoothSerial.readFromDevice(id);
    await Promise.all(response).then(console.log('Response Recieved ' + response));
...
export const getLightingStatus = async (id) => {
  ...
  try {
    ...
    if(response.length > 5 && response.substring(0,3)==="203"){
      ...
      //display check
      if(response.charAt(3) === 1) {  lightStatus.display = true }else if(response.charAt(3)=== 0){lightStatus.display = false};
      //extLight 
      if(response.charAt(4)=== 1) {  lightStatus.extLight = true }else if(response.charAt(4)=== 0){lightStatus.extLight = false};
...
export const getPrices = async (id) => {
  ...
  try {
    ...
    if(response.length > 5 && response.substring(0,3)==="208"){
      ...
      let prices = response.substring(3,19);
      var output = new Buffer(prices, 'hex');
      let priceOne = output.toString().substring(0,4);
      let priceTwo = output.toString().substring(4,9);
      console.log(priceOne, priceTwo)
      priceArray.push(priceOne);
      priceArray.push(priceTwo);
